...
export class ViewManager {
     ...
     * @returns 界面节点
     */
    public async open(cls: unknown, params?: unknown) {
        /** 界面控制类名 */
     ...
     * @returns 界面节点
     */
    public resume(cls: unknown, params?: any) {
        let className = this.getViewClassName(cls);
        if (!className || className === "") return;
     ...
     * @returns 界面节点
     */
    public close(cls: unknown, params?: any) {
        let className = this.getViewClassName(cls);
        if (!className || className === "") return;
    ...
    private getViewNode(cls: unknown) {
        let className = this.getViewClassName(cls);
        return this.viewNodes[className] || null;
    }


    private getViewScript(cls: unknown) {
        let className = this.getViewClassName(cls);
        let node = this.viewNodes[className];
        if (!node) {
    ...
    private getViewContainer(cls: unknown) {
        let className = this.getViewClassName(cls);
        if (cls === "string") cls = cc.js.getClassByName(cls);
        let containerType = cls["LAYER"];
        let viewContainer = LayerManager.getLayer(containerType);
        if (!viewContainer)
